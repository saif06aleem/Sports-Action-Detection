...
model_paths = {
    ...
    "Model5": "model_new_volleybask.h5",
}
...
class_names = {
    ...
    "Model5": ['pitching', 'saving', 'striking', 'dribble', 'drop', 'shoot', 'defending', 'goal_keeping', 'kicking',
               'block', 'defence', 'smash'],
}
    ...
    if result.pose_landmarks:
        ...
        if max(predict) > threshold:
            ...
            if model_name == "Model5":
                if pose_class in ['pitching', 'saving', 'striking']:
                    sport_name = "Baseball"
                elif pose_class in ['dribble', 'drop', 'shoot']:
                    sport_name = "Basketball"
                elif pose_class in ['defending', 'goal_keeping', 'kicking']:
                    sport_name = "Football"
                elif pose_class in ['block', 'defence', 'smash']:
                    sport_name = "Volleyball"

            cv2.putText(annotated_image, f'Pose : {pose_class}', text_pos_class, cv2.FONT_HERSHEY_SIMPLEX, font_scale,
                        (255, 255, 255), 1,
                        cv2.LINE_AA)
            cv2.putText(annotated_image, f'Sport: {sport_name}', text_pos_sport, cv2.FONT_HERSHEY_SIMPLEX, font_scale,
                        (255, 255, 255), 1,
                        cv2.LINE_AA)
...
def process_video(video_file, threshold, model_name):
    ...
    while cap.isOpened():
        ...
        if result.pose_landmarks:
            ...
            # Determine sport name based on pose class for Model5
            if model_name == "Model5":
                if pose_class in ['pitching', 'saving', 'striking']:
                    sport_name = "Baseball"
                elif pose_class in ['dribble', 'drop', 'shoot']:
            ...
            cv2.putText(annotated_frame, pose_class, (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2,
                        cv2.LINE_AA)
            cv2.putText(annotated_frame, f'Sport: {sport_name}', (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1,
...
sport_names = {
    ...
    "Model3": "Football",
    "Model4": "Volleyball",
    "Model5": ["Baseball", "Basketball", "Football", "Volleyball"]
}
...
def main():
    ...
    if file_type == "Image":
        ...
        if image_file is not None:
            ...
            if predict_button:
                result = process_image(image_file, threshold, model_name)
                if result is not None:
                    predictions, pose_class, output_img, annotated_image_path = result
                    ...
                    if pose_class in class_names[model_name]:
                        ...
                        st.write("Detailed Class Name:",
                                 class_names[model_name][class_names[model_name].index(pose_class)])
                    else:
        ...
        if video_file is not None:
            ...
            if predict_button:
                ...
                result = process_video(video_file, threshold, model_name)
                if result is not None:
                    output_frames, output_frames_path, output_video_path = result
